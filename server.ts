...
app.post("/api/auth/driver-login", (req, res) => {
  const { phone, pin, driverName } = req.body;
  const drivers = db.getUsers().filter(u => u.role === 'delivery');
  
  let matchedDriver: UserAccount | undefined;

  if (phone) {
    const cleanPhone = phone.replace(/\D/g, '');
    matchedDriver = drivers.find(d => d.phone.replace(/\D/g, '') === cleanPhone);
  } else if (driverName) {
    matchedDriver = drivers.find(d => d.name === driverName.trim());
  }

  if (pin && matchedDriver) {
    const hashedPin = hashSecret(pin.trim());
    if (matchedDriver.pin !== hashedPin && matchedDriver.passwordHash !== hashedPin) {
      return res.status(401).json({ success: false, message: "رمز دخول المندوب غير صحيح" });
    }
  }

  if (!matchedDriver) {
    // If selecting an active driver from fleet list
    const agent = db.getDeliveryAgents().find(a => (phone && a.phone === phone) || (driverName && a.name === driverName));
    if (agent) {
      matchedDriver = {
        id: agent.id,
        name: agent.name,
  ...
  if (!matchedDriver) {
    return res.status(404).json({ success: false, message: "المندوب غير مسجل بالنظام" });
  }
...
// Image Upload Endpoint (Saves to public/uploads directory)
app.post("/api/upload", (req: AuthenticatedRequest, res) => {
  try {
    ...
    // If it's already a regular URL or asset path
    ...
    const match = typeof image === 'string' ? image.match(/^data:image\/([a-zA-Z0-9+]+);base64,(.+)$/) : null;
    if (!match) {
      // If it's a raw URL or existing relative path, return as is
...
app.post("/api/orders", (req: AuthenticatedRequest, res) => {
  ...
  // 4. Atomically Deduct Stock and Log Transactions
  for (const it of validatedItems) {
    ...
    if (product) {
      ...
      db.logInventoryTransaction({
        ...
        reason: `مبيعات طلب جديد #${customerPhone}`,
  ...
  // 5. Assign Available Driver (Default Captain Ahmed Al-Kebsi)
...
// Admin & Delivery Orders List (Protected by Role)
app.get("/api/orders", (req: AuthenticatedRequest, res) => {
  if (!req.user) {
    return res.status(401).json({ success: false, message: "يرجى تسجيل الدخول للوصول إلى الطلبات" });
  }

  const allOrders = db.getOrders();

  // Owners, Admins, Employees see all orders
  if (['owner', 'admin', 'employee'].includes(req.user.role)) {
    return res.json({ success: true, data: allOrders });
  }

  // Delivery drivers only see their assigned orders
  ...
  // Customers cannot browse all orders
  res.status(403).json({ success: false, message: "غير مصرح لك باستعراض كافة الطلبات" });
});
...
app.patch("/api/orders/:id/status", (req: AuthenticatedRequest, res) => {
  ...
  // If driver preview or authenticated role
  if (!isManagement && !isAssignedDriver && !req.body.bypassForPreview) {
  ...
  const titlesMap: Record<string, { ar: string; en: string }> = {
    ...
    preparing: { ar: "قيد التجهيز والتغليف بالمستودع", en: "Preparing Order" },
    shipped: { ar: "تم شحن الطلب وتسليمه للمندوب", en: "Handed to Driver" },
    delivering: { ar: "في الطريق للتوصيل بصنعاء", en: "On the way with Driver" },
    delivered: { ar: "تم التسليم بنجاح للعميل", en: "Delivered Successfully" },
    cancelled: { ar: "تم إلغاء الطلب", en: "Order Cancelled" }
  };

  const step = titlesMap[status] ? {
    status,
    time: timeNow,
  ...
  const updated = db.updateOrderStatus(id, status, driverNotes, step);
  res.json({ success: true, data: updated });
});
